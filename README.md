...
Нужен **Docker Engine** + плагин **docker compose v2**. Прод-конфигурация лежит в
`docker-compose.prod.yml` (оверлей поверх базового файла): UI публикуется на порт `80`,
API — на `8000`, сервисы стартуют с `restart: unless-stopped`, API работает без
`--reload` и в несколько воркеров.

```bash
git clone https://github.com/kimpaili/nornickel_viral_laboratory.git
cd nornickel_viral_laboratory
cp .env.example .env
nano .env          # впиши YANDEX_API_KEY / YANDEX_FOLDER_ID и СМЕНИ POSTGRES_PASSWORD
chmod +x start.sh stop.sh
./start.sh         # сборка образа, up -d, seed, индексация корпуса, ожидание /health
```

После старта: **UI — `http://<сервер>:80`**, API/Swagger — `http://<сервер>:8000/docs`.
Остановить (данные БД сохраняются): `./stop.sh`.
...
docker compose -f docker-compose.yml -f docker-compose.prod.yml --profile tools run --rm corpus-index
```

Параметры сервера в `.env`: `APP_PORT` (публичный порт UI, по умолчанию 80),
`API_WORKERS` (число воркеров uvicorn), `POSTGRES_USER/PASSWORD/DB` (учётка БД).
Порты БД (`5433`) и UI-порт из base-файла (`8501`) привязаны к `127.0.0.1` — снаружи
сервера доступны только `80` и `8000`. За TLS поставь reverse-proxy (nginx/Caddy)
перед портом `80`.

Корпус литературы (`Задача 1/`) в репозиторий не входит (крупные PDF > лимита GitHub).
На сервере положи файлы в `Задача 1/` до `./start.sh` **или** загрузи их позже через
UI/эндпоинт `POST /corpus/upload` и переиндексируй:
`docker compose -f docker-compose.yml -f docker-compose.prod.yml --profile tools run --rm corpus-index`.
...
├─ docker-compose.prod.yml  # прод-оверлей для сервера (порт 80, restart, воркеры)
├─ start.bat / stop.bat     # запуск/остановка на Windows (dev)
├─ start.sh  / stop.sh      # запуск/остановка на Linux-сервере (prod)
└─ .env.example
```
