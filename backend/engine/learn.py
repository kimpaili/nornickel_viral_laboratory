...
def _next_coeff(
    ...
    Новый замер входит как один из (prior_count + 1) наблюдений: чем больше пилотов
    уже накоплено по правилу, тем меньше вес единичного замера — один нетипичный
    результат не может одним махом переписать коэффициент, выстроенный на десяти
    предыдущих, а первый эксперимент по новому правилу обновляет его сильно (усреднять
    не с чем). Провал — исключение из этой осторожности: он не сглаживается, а сразу
    тянет коэффициент к нижней границе диапазона, потому что для решения «не пилотировать
    это второй раз» избыточная осторожность лучше красивого усреднения.
    """
    ...
    if artifact.outcome == "failure":
        ...
        weight = Decimal(prior_count)
        candidate = (weight * before + observed) / (weight + Decimal("1"))
